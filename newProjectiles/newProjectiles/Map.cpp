...
GameMap::GameMap(sf::RenderWindow& window, int width, int height)
{
	...
	ground = sf::RectangleShape(Vector2f(10, 10));
	ground.setFillColor(sf::Color::Green);

	end = sf::RectangleShape(Vector2f(10, 10));
	end.setFillColor(sf::Color::Red);

	start = sf::RectangleShape(Vector2f(10, 10));
	...
	// Load map here
	tiles[30] = TileType::ground;
	tiles[39] = TileType::wall;
	tiles[0] = TileType::start;
	tiles[10] = TileType::wall;
	tiles[99] = TileType::end;
...
void GameMap::drawMap(sf::RenderWindow& window)
{
	...
	for (int y = 0; y < mapHeight; y++) {
		for (int x = 0; x < mapWidth; x++) {
			...
			case TileType::start:
				// Draw Ground
				start.setPosition(sf::Vector2f(draw_x, draw_y));
				window.draw(start);
				break;
			case TileType::end:
				// Draw Ground
				end.setPosition(sf::Vector2f(draw_x, draw_y));
				window.draw(end);
				break;
			case TileType::wall:
				// Draw Wall
